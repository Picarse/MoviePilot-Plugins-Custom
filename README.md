...
- 功能：向 MoviePilot“探索”注册优酷视频数据源，支持电视剧、电影、综艺、动漫、少儿和纪录片分类，并提供分频道题材、VIP/独播/首播、更新状态筛选
- 内容规模：汇总轮播、纵向推荐和 Feed 等全部首屏长视频模块；浏览完首屏节目后自动使用优酷匿名签名接口继续分页
- 可靠性：请求超时保护、频道和分页校验、广告及短视频过滤、节目去重、15 分钟频道缓存和 30 分钟分页缓存
- 独立性：使用独立插件 ID、配置前缀、API 路径、缓存区域和媒体 ID 前缀

插件优先使用每个频道约 77–105 个公开首屏长视频节目；超过首屏范围后，自动获取临时匿名令牌并按优酷网页相同规则生成签名。分页接口异常时安全返回空列表，不影响已经缓存的首屏内容。
