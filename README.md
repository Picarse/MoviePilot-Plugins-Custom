...
- 数据来源：优酷各频道公开的服务端渲染数据和匿名 Columbus 分页接口，无需登录或用户 Cookie
- 功能：向 MoviePilot“探索”注册优酷视频数据源，支持电视剧、电影、综艺、动漫、少儿和纪录片分类，并提供分频道题材、内容层级、VIP/独播/首播、更新状态筛选
...
- 可靠性：复用单次匿名令牌会话、请求超时保护、服务器 `more` 状态终止、频道和分页校验、广告及短视频过滤、全局节目去重、15 分钟频道缓存和 30 分钟聚合缓存
- 独立性：使用独立插件 ID、配置前缀、API 路径、缓存区域和媒体 ID 前缀
