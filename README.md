...
- 数据来源：优酷各频道页面公开的服务端渲染数据，无需登录或 Cookie
- 功能：向 MoviePilot“探索”注册优酷视频数据源，支持电视剧、电影、综艺、动漫、少儿和纪录片分类
- 可靠性：请求超时保护、频道和分页校验、广告及短视频过滤、节目去重、15 分钟频道缓存
- 独立性：使用独立插件 ID、配置前缀、API 路径、缓存区域和媒体 ID 前缀

优酷后续分页接口依赖动态签名和页面游标。本插件从每个频道的公开首屏结构化数据中提取约 70–80 个长视频节目，再由 MoviePilot 侧分页，避免依赖容易失效的签名算法。
...
刷新插件市场后，可搜索“自动删种（自用版）”“站点自动更新（自用版）”“腾讯视频探索（自用版）”或“优酷视频探索（自用版）”。
