...
| `page_url` | 爱奇艺官方节目页面 |
| `tag` | 地区、类型、年代、改编来源和自制等分号分隔标签 |
| `tag3lines` | 豆瓣高分、热度里程碑、奖项及榜单等卡片标签 |
| `theaters`、`awards` | 爱奇艺剧场品牌和获奖信息 |
| `starring`、`contributor`、`actor` | 演员名单（不同频道卡片使用不同字段） |
...
`?page=2` 会返回后续频道数据；电视剧当前返回一个带更丰富标签的瀑布流，后续页则可能返回 `code: 3`。`page_id`、`pg_num`、`channel_id` 和仅声明版本号不会产生相同分页效果。插件分别缓存频道响应 30 分钟；电视剧合并前两页中相同节目卡片的标签、演员和榜单归属后去重，再按截图架构执行类型、地区、时间、资费、殿堂、规格、奖项、剧场、演员、推荐和排序筛选及本地分页。

“殿堂”没有独立的节目字段，因此使用同一响应中的真实集合映射：荣誉殿堂为带获奖信息的节目，国民殿堂为 N 刷榜，人气殿堂为热播榜，佳片殿堂为高分榜。“近期转免/限免”来自卡片标签，“免费/VIP”来自 `pay_mark`，“自制/独播”分别来自 `tag` 和 `cornerMark`。中剧、短剧、短剧场及竖短片均不在插件入口和筛选项中提供。

以下路径验证为 404，不能当作可用频道：数字频道 ID，以及 `movie`、`anime`、`children`、`documentary` 等直译名称。`knowledge` 返回的是“知识-v7-PCW”，不是纪录片频道，插件没有把它错误映射为纪录片。

Mesh 是频道首页聚合接口，不接受 PC Web 分类目录那套数字分类参数，但节目卡片本身提供可用于本地筛选的标签。插件因此保留两种独立来源：

- “分类目录”继续负责六频道、地区、类型、年份、免费、制作归属及扩展分类筛选；
- “App频道”负责五频道栏目和榜单浏览，并为电视剧提供截图式多维筛选。
